...
from model.semseg.ChangeFormerBaseNetworks import *
import torch.nn.functional as F
...
class OverlapPatchEmbed(nn.Module): # embed_dims=[64, 128, 256, 512]
    """ Image to Patch Embedding
    """
    # Patch Embeeding操作将输入图像分成patch_size个的patch，from VIT
    ...
    def forward(self, x): # [4, 3, 256, 256]
        # pdb.set_trace()
        x = self.proj(x) # [4, 128, 32, 32]
        _, _, H, W = x.shape
        x = x.flatten(2).transpose(1, 2) # [4, 128, 1024] --> [4, 1024, 128]
        x = self.norm(x)
...
class EncoderTransformer_v3(nn.Module):
    ...
    def forward_features(self, x): # [4, 3, 256, 256]
        B = x.shape[0] # 4
        outs = []

        # stage 1
        x1, H1, W1 = self.patch_embed1(x) # [4, 4096, 64] [64] [64]
        for i, blk in enumerate(self.block1):
            x1 = blk(x1, H1, W1)
        x1 = self.norm1(x1) # [4, 4096, 64]
        x1 = x1.reshape(B, H1, W1, -1).permute(0, 3, 1, 2).contiguous() # [4, 64, 64, 64]
        outs.append(x1)

        # stage 2
        x1, H1, W1 = self.patch_embed2(x1) # [4, 1024, 128] [32] [32]
        for i, blk in enumerate(self.block2):
            x1 = blk(x1, H1, W1)
        x1 = self.norm2(x1) # [4, 1024, 128]
        x1 = x1.reshape(B, H1, W1, -1).permute(0, 3, 1, 2).contiguous() # [4, 128, 32, 32]
        outs.append(x1)

        # stage 3
        x1, H1, W1 = self.patch_embed3(x1) # [4, 256, 320] [16] [16]
        for i, blk in enumerate(self.block3):
            x1 = blk(x1, H1, W1)
        x1 = self.norm3(x1) # [4, 256, 320]
        x1 = x1.reshape(B, H1, W1, -1).permute(0, 3, 1, 2).contiguous() # [4, 320, 16, 16]
        outs.append(x1)

        # stage 4
        x1, H1, W1 = self.patch_embed4(x1) # [4, 64, 512] [8] [8]
        for i, blk in enumerate(self.block4):
            x1 = blk(x1, H1, W1)
        x1 = self.norm4(x1) # [4, 64, 512]
        x1 = x1.reshape(B, H1, W1, -1).permute(0, 3, 1, 2).contiguous() # [4, 512, 8, 8]
        outs.append(x1)
        return outs
...
class ChangeFormerV6(nn.Module):
    ...
    def forward(self, x1, x2):
        ...
        [fx1, fx2] = [self.Tenc_x2(x1), self.Tenc_x2(x2)]
        # [(4, 64, 64, 64), (4, 128, 32, 32), (4, 320, 16, 16), (4, 512, 8, 8)]
        cp = self.TDec_x2(fx1, fx2)
