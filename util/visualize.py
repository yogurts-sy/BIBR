...
vis_root = '/data/suyou/Codes/BIBR/visualize'
...
def visualize_pred_patches(tensor, tensor_pred, tensor_gt, base_path, name=''):
    ...
    # vis = np.concatenate([tensor], axis=1)
...
def visualize_bibr3(tensor_A, tensor_B, predict, predict_saved, predict_mixed, tensor_gt, base_path, name=''):
    img_margin_inner = np.zeros([256, 5, 3])
    batch_name = os.path.join(base_path, name + '.png')
    batch_size = 1
    ...
    for i in range(batch_size):
        ...
        P = make_numpy_grid(predict)
        PS = make_numpy_grid(predict_saved)
        PM = make_numpy_grid(predict_mixed)
        M = make_numpy_grid(tensor_gt)
        vis = np.concatenate([A, img_margin_inner, B, img_margin_inner, P, img_margin_inner, PS, img_margin_inner, PM, img_margin_inner, M], axis=1)
        if batch_img.shape == (0,):
            batch_img = vis
        else:
            batch_img = np.concatenate([batch_img, vis], axis=0)
...
def visualize_hotmap(predict, name='', dist_dir='hotmap'):
    ...
    for i in range(2):
        ...
        fig, ax = plt.subplots()
        im = ax.imshow(H, cmap='Reds', interpolation='nearest')
